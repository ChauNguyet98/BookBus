...
export class ListSeatComponent implements OnInit {
  ...
  onBookSeat(SoGhe, TrangThai) {
    var index = this.arrSeats.findIndex(seat => seat.SoGhe === SoGhe);
    if (TrangThai === false)
    {
      this.arrSeats[index].TrangThai = true;
      this.myClick.emit(this.arrSeats[index].SoGhe);
      alert("Book seat number "+ SoGhe + " successful");
    }
    else {
      alert("Seat number "+ SoGhe + " is booked.");
    }
  }
